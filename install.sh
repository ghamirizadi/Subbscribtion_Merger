...
OUT_FILE = Path("/var/www/sub/1111.text")
...
def uninstall_everything():
    ...
    run(["rm", "-f", "/etc/systemd/system/v2subagg.service", "/etc/systemd/system/v2subagg.timer"])
    run(["systemctl", "daemon-reload"])
    run(["rm", "-f", "/etc/nginx/sites-enabled/sub", "/etc/nginx/sites-available/sub"])
